...
exports.artist_update_get = function(req, res, next) {
  async.parallel({
    ...
    if (err) { return next(err); }
    res.json( { artist: results.artist});
  });
}

exports.artist_update_put = function(req, res, next) {
  if(!req.body || !req.body.title || !req.body.synopsis || !req.body.body ) {
    return errorHandler.handleAPIError(400, `Artist must have a title, synopsis, body`, next);
  }
  ...
  Artist.findByIdAndUpdate(id, {
    ...
    .then(artist => {
      ...
      return errorHandler.handleAPIError(500, `Could not update artist with id: ${id}`, next);
    });
}

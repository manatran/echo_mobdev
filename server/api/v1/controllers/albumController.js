...
exports.album_update_get = function(req, res, next) {
  async.parallel({
    ...
    if (err) { return next(err); }
    res.json( { album: results.album});
  });
}

exports.album_update_put = function(req, res, next) {
  if(!req.body || !req.body.title || !req.body.synopsis || !req.body.body ) {
    return errorHandler.handleAPIError(400, `Album must have a title, synopsis, body`, next);
  }
  ...
  Album.findByIdAndUpdate(id, {
    ...
    .then(album => {
      ...
      return errorHandler.handleAPIError(500, `Could not update album with id: ${id}`, next);
    });
}
